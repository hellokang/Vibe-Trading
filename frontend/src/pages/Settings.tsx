...
export function Settings() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`${t("settings.saveLlmFailed")} ${error instanceof Error ? error.message : "Unknown error"}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`${t("settings.saveDataFailed")} ${error instanceof Error ? error.message : "Unknown error"}`);
    } finally {
      setDataSaving(false);
    }
  };
  ...
  const localApiAccessSection = (
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{t("settings.localApiDesc")}</p>
      </div>
      <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto]">
        <label className="grid gap-2">
          ...
          <input
            ...
            placeholder={t("settings.localApiPlaceholder")}
      ...
      <p className="mt-2 text-xs text-muted-foreground">{t("settings.localApiPlaceholder")}</p>
    </form>
  );
  ...
  const keyStatus = settings.api_key_configured
    ? t("settings.apiKeyStatusConfigured")
    : settings.api_key_required
      ? t("settings.apiKeyStatusKeep")
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? `${t("settings.apiKeyStatusOAuth")} ${selectedProvider.login_command}`
        : t("settings.apiKeyStatusNotRequired");
  ...
  const tushareStatus = dataSettings.tushare_token_configured
    ? t("settings.apiKeyStatusConfigured")
    : t("settings.tushareTokenStatusKeep");
      ...
      <form onSubmit={submit} className="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.8fr)]">
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              <span className={labelClass}>{t("settings.modelName")}</span>
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <select
                ...
                className={fieldClass}
              >
                <option value="">{t("settings.reasoningEffortOff")}</option>
                <option value="low">low</option>
                <option value="medium">medium</option>
                <option value="high">high</option>
                <option value="max">max</option>
              </select>
              <span className={hintClass}>{t("settings.reasoningEffortHint")}</span>
            </label>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{t("settings.tushareDesc")}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
            ...
            >
              ...
              {dataSaving ? t("settings.saving") : t("settings.saveDataSources")}
            </button>
          </div>
